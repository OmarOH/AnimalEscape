namespace Assets.Scripts.Water
{
    ...
    {
        [SerializeField] private GameObject[] waterRenderers;
        ...
        public void Awake()
        {
            ...
            waterRenderers = GameObject.FindGameObjectsWithTag("Water");

            if (waterRenderers != null)
            {
                for (var i = 0; i < waterRenderers.Length; i++)
                {
                    waterRenderers[i].GetComponent<Renderer>().SetPropertyBlock(materialPropertyBlock);
                }
            }
        }
